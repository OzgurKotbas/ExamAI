...
def _build_cache_key(user_id: str, note_id: str, total_questions: int, mc_ratio: float, difficulty: str) -> str:
    """Deterministic SHA-256 cache key."""
    raw = f"{user_id}:{note_id}:{total_questions}:{mc_ratio:.2f}:{difficulty}"
...
async def set_cached_quiz(
    user_id: str, note_id: str, total_questions: int, mc_ratio: float, difficulty: str,
    quiz_id: str, ttl_seconds: int = 86400,
) -> str:
    ...
    try:
        ...
        logger.info(f"Cache SET for key: {key}, quiz_id: {quiz_id}, TTL: {ttl_seconds}")
        return key
