...
from utils.security import hash_password, verify_password, create_access_token, decode_access_token, encrypt_text
...
async def request_password_reset(db: AsyncSession, email: str, lang: str = "tr") -> Tuple[bool, str]:
    ...
    SMTP varsa gerçek e-posta gönderilir.
    SMTP yapılandırılmamışsa (development/test ortamı) kod Redis'te saklanır
    ve geliştirici loguna yazılır; kullanıcıya yine de "kod gönderildi" mesajı
    verilir. Bu sayede SMTP olmadan da akış test edilebilir.
    """
    try:
        # Check if user exists
        result = await db.execute(select(User).where(User.email == email))
        ...
        if not user:
            # Don't reveal whether the user exists — always respond positively
        ...
        # Generate and store reset code in Redis (10-min TTL)
        reset_code = generate_reset_code()
        if not store_reset_code(email, reset_code):
            raise ValueError("Failed to store reset code in Redis")

        # Attempt to send email via SMTP
        email_sent = send_password_reset_email(email, reset_code, user.full_name)

        if not email_sent:
            # SMTP not configured or failed — decide by environment
            if settings.is_production:
                # In production we MUST send the email; fail hard
                delete_reset_code(email)
                logger.error(
                    f"[PRODUCTION] Password reset email could not be sent to {email}. "
                    "Verify SMTP_USER, SMTP_PASSWORD, SMTP_HOST and SMTP_PORT env vars."
                )
                raise ValueError(
                ...
                # Development/staging: log the code so it can be used without real SMTP
                logger.warning(
                    f"[DEV] SMTP not configured — password reset code for {email}: {reset_code} "
                    "(10-min TTL, stored in Redis)"
                )
                # Still return success so the frontend can proceed to the verify step

        logger.info(f"Password reset flow completed for: {email} (email_sent={email_sent})")
...
async def update_user_profile(db: AsyncSession, user: User, full_name: str, email: str, gemini_api_key: str | None = None, gemini_model: str | None = None, lang: str = "tr") -> User:
    ...
    try:
        ...
        # Encrypt Gemini API key before storing (AES-256-GCM)
        encrypted_api_key: str | None = None
        if gemini_api_key and gemini_api_key.strip():
            try:
                encrypted_api_key = encrypt_text(gemini_api_key.strip())
            except Exception as enc_err:
                logger.error(f"Failed to encrypt Gemini API key: {enc_err}")
