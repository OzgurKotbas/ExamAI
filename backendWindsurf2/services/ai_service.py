...
async def validate_gemini_api_key(api_key: str, model_name: str, lang: str = "tr") -> dict:
    ...
    try:
        ...
        if "invalid project id" in body_text or "project id invalid" in body_text:
            logger.warning(f"API key validation failed – invalid project id: {body_text}")
            return {"valid": False, "message_key": "GEMINI_PROJECT_ID_INVALID"}
        ...
        if "invalid project id" in str(e).lower() or "project id invalid" in str(e).lower():
            logger.error(f"API key validation failed – invalid project id: {e}")
            return {"valid": False, "message_key": "GEMINI_PROJECT_ID_INVALID"}
        ...
        try:
            ...
            for model_name in models_to_try:
                try:
                    ...
                    if any(err in error_text for err in ["401", "403", "api_key_invalid"]):
                        raise
                    if "invalid project id" in error_text or "project id invalid" in error_text:
                        raise ValueError(translate("GEMINI_PROJECT_ID_INVALID", lang))
                    last_exception = e
                    continue
            ...
            if any(err in error_text for err in ["401", "403", "api_key_invalid"]):
                ...
                raise
            if "invalid project id" in error_text or "project id invalid" in error_text:
                logger.error(f"Invalid Gemini project ID: {e}")
                raise ValueError(translate("GEMINI_PROJECT_ID_INVALID", lang))
            last_exception = e
...
def _log_prompt(prompt_type: str, prompt: str, response: str = "", provider: str = "unknown"):
    """
    Log the AI interaction type and provider for auditing.
    Only the first 300 characters of the prompt and response are written to
    disk to avoid storing sensitive user note content in plain-text log files.
    """
    import os
    ...
    try:
        ...
        prompt_snippet = prompt.strip()[:300]
        response_snippet = response.strip()[:300] if response else ""

        with open(log_file, "a", encoding="utf-8") as f:
            f.write(f"================================================================================\n")
            f.write(f"[{timestamp}] TYPE: {prompt_type} | PROVIDER: {provider}\n")
            f.write(f"--- PROMPT (first 300 chars) ---\n")
            f.write(f"{prompt_snippet}{'...[truncated]' if len(prompt.strip()) > 300 else ''}\n")
            if response_snippet:
                f.write(f"--- RESPONSE (first 300 chars) ---\n")
                f.write(f"{response_snippet}{'...[truncated]' if len(response.strip()) > 300 else ''}\n")
            f.write(f"================================================================================\n\n")
    except Exception as e:
