...
from services.ai_service import _get_hf_tokens
from services.cache_service import _build_cache_key, get_cached_quiz
from utils.security import decrypt_text, safe_decrypt_api_key
...
# Rate limiter — shared instance from main.py
# Rate limiter — döngüsel import'u önlemek için limiter.py'den alınır
...
async def create_quiz(
    ...
    """
    Create a new quiz from a note. Rate limited: 10/minute per IP.
    ...
    """
    try:
        # API key: kullanıcı anahtarı veya sistem fallback (Gemini/HF) gerekli
        user_api_key = safe_decrypt_api_key(current_user.gemini_api_key)
        has_system_gemini = bool(settings.GEMINI_API_KEY and settings.GEMINI_API_KEY.strip())
        has_hf_fallback = bool(_get_hf_tokens())
        
        # Determine API source and check free quiz limit
        api_source = None
        if user_api_key:
            api_source = "user_gemini"
        elif has_system_gemini:
            api_source = "system_gemini"
        elif has_hf_fallback:
            api_source = "hugging_face"
        
        # Free quiz limit: users without API key can create max 3 quizzes
        if not user_api_key:
            if current_user.free_quiz_count >= 3:
                logger.warning(
                    "Quiz creation blocked: user has exceeded free quiz limit (3) | user=%s",
                    current_user.id,
                )
                raise HTTPException(
        ...
        if not user_api_key and not has_system_gemini and not has_hf_fallback:
            logger.warning(
                "Quiz creation blocked: no user API key and no system AI fallback | user=%s",
                current_user.id,
            )
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST,
                translate("GEMINI_KEY_MISSING", lang),
            )
        ...
        # Increment free quiz count for users without API key
        if not user_api_key:
            current_user.free_quiz_count += 1
            logger.info(f"Incremented free_quiz_count for user {current_user.id} to {current_user.free_quiz_count}")
